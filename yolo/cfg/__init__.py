...
from yolo.utils import ASSETS, DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, FLOAT_OR_INT, IS_VSCODE, LOGGER, RANK, ROOT, RUNS_DIR, SETTINGS, SETTINGS_FILE, STR_OR_PATH, TESTS_RUNNING, YAML, IterableSimpleNamespace, colorstr, deprecation_warn, vscode_msg
...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    """
    from yolo.solutions.config import SolutionConfig
...
def entrypoint(debug: str = "") -> None:
    ...
    if "rtdetr" in stem:  # guess architecture
        from yolo import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from yolo import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem or "sam2.1_" in stem:
        from yolo import SAM
